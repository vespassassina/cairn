...
// remainder, so the total budget carries that cost explicitly. About 600
// tokens total; see pnpm context-cost.
export const INSTRUCTIONS_BUDGET = 2400;

/** The fixed text's own ceiling, so it can never eat the summary's room. */
export const FIXED_INSTRUCTIONS_CEILING = 1500;
...
Pages are private. Only the owner publishes one; there is no tool for it, so say that when asked. Never store secrets, credentials or throwaway scratch work.`;
