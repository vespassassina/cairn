...
| Publish the CLI to npm | next | Published as `cairncli` (2026-09-15), renamed from `@cairn/cli` after the `cairn` npm org turned out to be taken. `packages/cli/package.json` is no longer private and CI's release job publishes it on a version tag, alongside the GitHub release it already made. Retrying against `v0.1.5` after the token and name fixes (`docs/LESSONS.md`) |
