...
| Item | Status | Notes |
|---|---|---|
| Presence: a session hook, a job that starts the local Cairn, and `cairn status` | next | ADR-053, `docs/specs/presence.md`. `cairn hook install` adds a Claude Code SessionStart hook running a bounded `cairn overview --brief`; the scheduled job runs `cairn start` instead of `cairn sync`, which needs no new code because `start` already starts what is not answering and then syncs; `cairn status` reports instance, sign-in, last sync, embeddings, job and hook in one screen, each bad line carrying its fix |
| The sign-in that deletes itself | next | ADR-054, `docs/specs/sign-in-resilience.md`. Two defects compound: the server answers `invalid_grant` to a concurrent refresh that arrives between `takeAuth` and the replay write, and the CLI then deletes the credentials on any error at all, including a timeout on an Azure cold start. A losing process erases the winning process's fresh tokens. The server waits up to a second for the answer another request is writing; the CLI forgets only on a parsed `invalid_grant`, re-reads before writing, and gives the refresh a 30 second timeout. Access tokens stay at one hour, per ADR-033 |
| The workspace summary gets its own budget | next | ADR-055, part of `docs/specs/agent-navigation.md`. The fixed instructions are 1,895 of a 2,200 character budget, so the live summary gets 303 and names no collections at all on a real workspace. The summary gets a 700 character floor, the fixed text a 1,500 character ceiling enforced by a test, and the total rises to 2,400. The test builds a summary for a workspace the size of a real one, which is what the current tests do not do |
...
| Two weeks of daily use, against the PRD kill criterion | blocked | The review of 2026-09-16 found the criterion was being measured against an instrument that was switched off. The clock restarts once the presence work (ADR-053) and the sign-in fix (ADR-054) are in place, because until then a failure to reach for Cairn cannot be told apart from a failure to reach Cairn |
