...
| A local trusted friends catalog | later | 2026-09-15, from the owner's direction: "a cairn can keep references to other cairns and act as a local 'trusted friends' catalog". Each Cairn keeps its own list of other Cairns it trusts, kept locally rather than shared or published anywhere. Needs an ADR: whether the list lives in server data or CLI config, what a trusted entry records beyond an address, and how it is managed (console, CLI, MCP) |
