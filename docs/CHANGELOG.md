...
**The project guide now carries the owner's way of working.** The owner asked, after sprint 2, to "update the process and dev guide from my global one". `CLAUDE.md` gains "How work is done": design interview, spec with acceptance criteria, plan in `docs/PLAN.md` with one outcome and one test per task, test first, one sprint at a time with review and re-planning between, `docs/HANDOFF.md` when stopping midway, the skills to reach for, and the owner's Cairn as the place for persistable knowledge. The workflow section adds the commit rules (imperative subject, a body that says why, the no-reply author, the co-author line). The verification list adds the root-run note, `pnpm smoke:cli` for CLI changes, a browser check per state and width for console changes, and an adversarial pass. The coding style adds the simplicity rules; the writing style adds the language, unit and date standards. The current-phase paragraph names the 2026-09-24 plan and where it stands. Why: the rules were applied through the three sprints of `docs/PLAN.md` but lived only in the owner's private file, so another agent, or Claude in a fresh session without it, could not have followed them. `AGENTS.md` and `docs/README.md` point at the same rules. No code changed.

## 2026-09-25

**Sprint 2, task 7 (ADR-079): the guides, and sprint 2 closes.** `docs/AGENT-OPERATE.md` gains section 9, "The Inbox and drop tokens": what the Inbox is, how to file what waits and never to leave a used drop there, what a drop token can and cannot do, that the kind is recorded on every drop, and that files need `CAIRN_ATTACHMENTS_TO`; the sections after it moved down one, and the settings table's pointer to the rotation section followed. `docs/CLI.md` gains "Throw things in for later: drop, drops and drop tokens" with the commands, the `curl` lines for a phone or a script, the iPhone Shortcut and Android share-sheet recipe, and the HEIC note (spec risk 3). `docs/LOCAL.md` section 6 names the console's `/inbox` and `/settings/drop-tokens`, `docs/DEPLOY-AZURE.md` "Day to day" gains "Drop from your phone", and the PRD's MCP paragraph says the Inbox exists and has no tool of its own. Test for criterion 8, in `packages/api/test/drops.test.ts`: a drop is found by search, and once an agent has linked it from the target page and moved it there, the drop's backlinks name that page and `listDrops` is empty. The roadmap row is done and the plan says sprint 3 is next. One lesson recorded: a newest-first test that creates two records in the same millisecond has no order to assert.

**Sprint 2, task 6 (ADR-079): agents learn about the Inbox.** The workspace summary gains one line, `Inbox: N drops waiting to be filed.`, right after the approval counts and only when N is above zero, from the same `dropsAmong` the console and `GET /drops` use, so every surface reports the same number. The server instructions gain one paragraph (documentation rule 8, ADR-079 decision 5): the Inbox holds drops for the agent to file; read each, search for where it belongs, append or create there with a change note naming the drop, then move the drop under that page or delete it once absorbed, and never leave a used drop in the Inbox. The skill says the same in the CLI's words in a new section, "The Inbox", with `cairn drops` and `cairn drop`. The fixed instructions' ceiling goes from 1900 to 2200 characters and the total budget from 2800 to 3100 (ADR-055's numbers, the third time a paragraph has moved them); the paragraph was cut to fit, since the first draft was twelve characters over. `pnpm context-cost` moves from about 7,300 to about 7,400 tokens per MCP session; the README says so. Tests: a summary case for the line, and the existing MCP budget test, which checks every tool name the instructions mention exists, passes with the four new mentions.

**Sprint 2, task 5 (ADR-079): `cairn drop`, `cairn drops`, `cairn drop-token`.** `cairn drop "text" [--title T] [--tag X]... [--file PATH]... [--url U]` sends one multipart request to `POST /api/v1/drops`, the text from the argument, `--text` or stdin, each file read with `node:fs/promises` and sent as a `File` in a `FormData`, which both Node and the Bun executable provide (hard rule 16; `pnpm smoke:cli` now drops and lists on the compiled binary). A 26 MB file is refused by the server's `413 drop_too_large` and the CLI prints that message, which names the file and the 25 MB ceiling; nothing to drop is a usage error naming the three ways to give text. `--file` became repeatable, so `cairn create --file` and `cairn attachment create --file` take its first value. `cairn drops` lists what waits, newest first, with ids and how to file or delete one; it reads the new `GET /api/v1/drops`, backed by `listDrops` in `operations.ts`, which shares `dropsAmong` with the console's `/inbox` and home count so the three agree. `cairn drop-token create <name> [--description D] [--kind person|agent]` prints the token once with a `curl` line that uses it and says when the kind defaulted to person; `list` shows kind and last use and never the value; `revoke` says what happens to whoever still holds it. Tests: `packages/cli/test/drop-command.test.ts` (round trip, 25 MB accepted and 26 MB refused with the limit, attachments off, the token flow through an app without local trust since a trusted localhost caller is the owner whatever it carries) and a `GET /drops` case in `rest.test.ts`.

**Sprint 2, task 4 (ADR-079): the Inbox in the console.** `/inbox` lists the drops newest first, each with who dropped it and when, the first lines of its text without the file links `createDrop` appended, a thumbnail per image and a name pill per other file, and two forms: "File under" with a page picker, and Delete. The picker offers every page but the Inbox and the other drops, since filing a drop under a drop is never the aim. Filing posts to the new `POST /p/:id/move`, deleting to the new `POST /p/:id/delete`, both plain forms with the version the person saw and a `next` to return to; the same `moveRecord` and `deletePage` the MCP tools call, so hard rule 14 holds. A page with children is not deleted and the answer says why. `/inbox/new` is the desktop capture form, multipart, with text, files, title, tags and a link; it checks the limits before reading a file's bytes, takes a drop slot like the API, answers 413 with the ceiling or 422 naming `CAIRN_ATTACHMENTS_TO` with the text kept in the form, and returns to the Inbox. The home page says "N drops waiting" and links there when there is at least one; the section nav gains Inbox. Thumbnails come from a new console route, `GET /a/:id/thumb`, that streams the webp bytes from the blob store through `getAttachmentThumbnail`: the console's CSP allows images from its own origin only, and widening it to signed blob URLs was the alternative not taken. `/settings/drop-tokens` creates a token (name, description, person or agent), shows the value once in a banner and never again, lists the rest with last use, and revokes in place; a blank name gets a 400 with the message from `createDropToken`. Tests: four cases under "the Inbox in the console" in `packages/api/test/console.test.ts`, covering criterion 5 and the token page. Checked in the browser on the scratch server at desktop and phone width. The base CSS for the drop rows sits above the phone block, per the lesson of task 8.

**Sprint 2, task 3 (ADR-079): drop tokens.** New `packages/api/src/drop-tokens.ts`, the shape of `publish-tokens.ts`: a well-known table "Drop tokens" with `name`, `description`, `kind` (person or agent, chosen at creation), `token_hash`, `last_used_at` and `revoked_at`, created on first use. A token is `cairn_drop_` plus 32 random bytes base64url, returned once by `createDropToken` and stored as its SHA-256; no list, row or answer ever carries the value again (criterion 4, hard rule 18). The app's auth check routes a bearer token with that prefix before the service token and OAuth: unknown answers 401 saying how to make one, revoked answers 401 naming the token and the date, and anything but `POST /api/v1/drops` answers `403 drop_token_scope` naming the one route it opens. A valid one sets the caller to the token's kind with the label `token "<name>"` and moves `last_used_at`, best effort, so a leaked token shows up in the list (decision 4). The drop's default revision note becomes `Dropped via token "<name>"`. REST gains `POST /drop-tokens`, `GET /drop-tokens` and `POST /drop-tokens/:id/revoke` for the CLI and the console; there is no MCP tool, as the ADR says. Tests: `packages/api/test/drop-tokens.test.ts` and the `drop tokens` block in `rest.test.ts`.

**Sprint 2, task 2 (ADR-079): `POST /api/v1/drops`.** The endpoint takes `multipart/form-data` (`text`, `title`, `tags` as a comma list, `url`, any number of `files`, which is what a share sheet and `curl -F` send) or JSON with the same fields and files as base64 `data`, and answers 201 with the page summary plus `link`, the console address of the drop. It calls `createDrop` and nothing else decides. Limits (decision 2) are one function, `checkDropLimits`, run on sizes before any file's bytes are read: over 10 files, over 25 MB in a file or over 64 KB of text answers `413 drop_too_large` with the ceiling and the way round it in the message, and a `Content-Length` past the whole budget is refused before parsing. At most two drops pass through the process at once, whichever surface they arrive from: `withDropSlot` queues the rest rather than refusing them, because a phone that shared three photos expects three pages. Attachments-off now has its own error code, `attachments_off`, with `setting: "CAIRN_ATTACHMENTS_TO"`, on this endpoint and on `POST /attachments` alike, since the fix is a setting and an agent should not retry with different input; the message is unchanged. Tests: the `drops (ADR-079)` block in `packages/api/test/rest.test.ts`, with a real multipart request, a JSON one, the three limits, the two-slot cap and attachments off. Session-cookie drops arrive with the console form in task 4; drop tokens in task 3.

**Sprint 2, task 1 (ADR-079): the Inbox and `createDrop`.** Gate 2 of `docs/PLAN.md` passed on the numbers recorded for sprint 1 (recall@5 unchanged on both backends), so sprint 2 opens and ADR-079 moves to accepted, as does ADR-078 now that its sprint is done. `INBOX_COLLECTION_NAME = "Inbox"` joins Templates and Daily notes in `packages/api/src/templates.ts`, found by title and created on first use (`findOrCreateCollection` is now exported for it). `createDrop` in `packages/api/src/operations.ts` is the one operation every surface of sprint 2 will call: it files an ordinary page under the Inbox, titled by `dropTitle` (the `title` field, else the first non-blank line of the text cut at 80 characters, else the first filename, else `2026-09-24 19:31`), tagged with what was sent plus `drop`, with the `url` as its one source, and with each file linked from the body after a blank line by `dropBody` (image links for the raster types ADR-064 thumbnails). Files are stored by the new `storeAttachment` in `attachments.ts`: the server hashes the bytes with Web Crypto, writes the pending row, puts the blob and runs the same `confirmAttachmentUpload` a direct upload ends with, so what lands is indistinguishable from one; the checks `createAttachment` made inline are now `checkAttachmentInput`, shared by both. Order in `createDrop` is deliberate: attachments-off is refused before anything is written, the page is created with the text alone because an attachment row needs a page, then the files land, then the body gets its links in one more write with the same note. Test: `packages/api/test/drops.test.ts`. The endpoint, limits and tokens follow in tasks 2 and 3.

## 2026-09-24

**Sprint 1, task 10 (ADR-078): the three texts, the guides and the numbers.** The MCP server instructions, the workspace summary and the skill now say the same thing about the mark (documentation rule 8, ADR-078 decision 5): only the owner sets it and there is no tool, so say so when asked; search ranks approved pages up and hides disapproved ones unless `include_disapproved` is passed; a disapproved page's notice means do not build on it, and a changed page waits for the owner's re-review. The instructions gained one paragraph, so their ceiling went from 1500 to 1900 characters and the total budget from 2400 to 2800 (ADR-055's numbers), and `pnpm context-cost` moved from about 7,200 to about 7,300 tokens per MCP session; the README says so. The summary adds one line, `Approval: N approved, N changed since approval, N disapproved.`, from the same `approvalCounts` the console uses, and only once there is a mark, so a workspace without one reads as before. The skill has a section "The owner's mark" with the CLI's words for the same three sentences. `docs/AGENT-OPERATE.md` gains section 8, "The approval mark": you cannot approve, where the review queue is, what agents do with the mark, when an edit resets it, and that it travels; the sections after it moved down one. The PRD's MCP paragraph and the roadmap row say the feature is built. Sprint 1 is complete; gate 2 is the owner's review of the eval numbers under task 5. Tests: a summary case for the count line, and the MCP budget test learned the two new field names are not tools.

**Sprint 1, task 9 (ADR-078): sync, export and import carry the mark.** The mark is knowledge about the content, so it travels with it, unlike publication (decision 7). Sync's page content, the shape it hashes and merges, now includes `approval` and `approval_at` when a page is marked, and `approval_previous` when an edit reset a mark, so a page that is neutral with no trace hashes as before and an upgrade finds nothing to copy. Export writes the same fields into the front matter; import reads them and sends them, and an export from before the mark existed leaves the target's mark alone. A merge treats the three fields as one value, and when both sides marked a page the later mark wins. What does not travel is `approval_version`: it names a revision of the server that set the mark, which the other server never had. Instead the copy route, `PUT /pages/:id`, takes `approval`, `approval_at` and `approval_previous` and points the mark at the version it writes, through a new core flag `approvalOfThisWrite`, so "Changes since approval" works on both Cairns and the carry-over rule has a baseline there. A write that names the mark skips the carry-over rule, which is right for a copy: the source already applied it. The copy route trusts its caller for the mark as it does for the body, because `cairn sync` runs from agent sessions too; the person-only rule holds for `setApproval`, and the copy's revision note names the server it came from. Tests: one case each in `packages/cli/test/sync.test.ts` (mark, reset and unmark across two servers, mark pointing at the copy's version), `export.test.ts` and `merge.test.ts`.

**Sprint 1, task 8 (ADR-078): side-by-side diff.** Every version page takes `?view=side` and renders the same diff as two columns, older text left, newer right, each removed line beside the added line that replaced it and the longer side's extra lines facing an empty cell. A "Side by side" or "Inline" button switches. The folding is one shared function, `foldedRuns` in `layout.tsx`, so both views show the same lines. `?from=<version>` compares with that version instead of the one before; "Changes since approval", on the page view and in the review queue, opens the side view between the approved version and the current one, headed "What changed since it was approved". CSS grid, no script; under 700 px the rows stack to one column, before over after. The stylesheet's phone block sits before the diff rules in the file, so the side-by-side base rules moved above it: same specificity, so order decides, and the first try rendered two columns on a phone. The test now checks that order. Tests: three cases under "side-by-side diff" in `packages/api/test/console.test.ts`, checked in the browser at both widths.

**Sprint 1, task 7 (ADR-078): the mark in the console.** Three things. The page view shows where a page stands (approved or disapproved by whom and when, "was approved, changed since", or "not reviewed"), a red banner with the same notice agents get on a disapproved page, an amber one on a page changed since approval, and Approve, Disapprove and Unmark buttons in the header, each a plain form posting to `POST /p/:id/approval` with the version the person looked at, so they work with scripts off; a stale version gets a 409 page saying to open the page again. `/review`, in the section nav, is the queue from decision 6: pages whose mark an edit reset, then unmarked pages last written by an agent, newest first, then the rest by inbound links, with the same buttons per row returning to the row. The home page counts what is waiting and links there. The queue and the counts come from one shared operation, `reviewQueue` in `operations.ts`, so the workspace summary (task 10) will say the same numbers. The edit form on a marked page offers "Keep the mark even if this is a large change", which names the mark in the write and so skips the carry-over rule; the revision note says `(approval kept by the owner)`. The section nav now wraps at phone width, where the fifth link no longer fit. Tests: seven cases under "approval in the console" in `packages/api/test/console.test.ts`, one polling for the derived link count (hard rule 11). Checked in the browser on the scratch server at desktop and phone width.

**Sprint 1, task 6 (ADR-078): reads show the mark and the notice.** `pageSummary`, which every MCP page read and the REST page JSON share, now carries `approval` on every page, and `approval_at`, `approval_version`, `approval_previous` only when set, so an unmarked wiki pays nothing per read. A disapproved page, or a neutral one that was marked before an edit reset it, carries one more field, `approval_notice`, placed before `body`: "Disapproved by the owner on <date>. Do not build on this page; say so if asked about it." or "Was approved; changed since by <who> on <date>, not yet re-reviewed." The line is a separate field, never prepended to the body, so an agent that reads, edits and writes back cannot copy it into the page. The REST Markdown format and `cairn read` print an `approval:` line in the front matter (with `(was approved)` when the mark was reset) and the notice as a quoted line between the front matter and the body. The wording lives in one place, `approvalNotice` in `operations.ts` (hard rule 14). Tests: `packages/api/test/approval-read.test.ts` (MCP, REST JSON, REST Markdown, field order, body untouched) and a read case in `packages/cli/test/approve-command.test.ts`.

**Sprint 1, tasks 4 and 5 (ADR-078): search honours the mark.** `searchPages` now reads each hit page's mark once per search, multiplies an approved page's score by `APPROVED_BOOST = 1.25`, drops disapproved pages unless `includeDisapproved` is passed, and re-ranks by score with the index's order kept for ties. Every `PageHit` carries `approval`. Surfaces: MCP `search` gains `include_disapproved` and returns `approval` per page; REST `GET /search` takes `include_disapproved=1` and returns `approval`; the CLI takes `--include-disapproved` and prints `[approved]` or `[disapproved]` after the page id; the console passes nothing, so it hides disapproved pages too. Hard rule 14: the console was already going through `searchPages`, and `parity.test.ts` now checks statically that it never calls the index directly. `pnpm eval` before and after, SQLite backend: keyword recall@5 0.88 to 0.88, hybrid 0.97 to 0.97, pages@5 1.46 and 2.27 unchanged. That is expected and says less than it seems: the eval measures the raw chunk index beneath `searchPages` (ADR-057 consequence 3), where the boost does not apply, and no page in the eval workspace carries a mark. Whether 1.25 moves anything for an agent is a question for the review queue, not this eval (ADR-078 consequence 4). Tests: `packages/api/test/approval-search.test.ts` (ranking, filter, and the same order through MCP, REST and the console) and a CLI case in `packages/cli/test/approve-command.test.ts`. `pnpm smoke:cli` passes.

**Sprint 1, task 3 (ADR-078): the mark survives a small edit and resets on a large one.** New `packages/core/src/approval.ts`: `CARRY_OVER` (5 % of the approved body, never more than 200 characters), `changedChars` (line diff, each replaced line paired with its replacement and only the differing part counted, unpaired lines whole) and `approvalAfterEdit`. The core page write applies it to every edit of a marked page that does not name the mark itself: kept, the note gains `(approval kept: small change)`; reset, `approval` becomes neutral, `approvalPrevious` remembers the old state, `approvalAt` and `approvalVersion` stay as the diff baseline, and the note gains `(approval reset: was approved)`. The measure is always against the body at `approvalVersion`, so three small appends add up and reset, as the spec's success test requires. A write that names `approval`, as `setApproval`, sync and import do, is left alone. The ADR said the rule and its constants live in `operations.ts`; they live in core instead, because the console, sync and import call the core service directly and would have bypassed the rule. ADR-078 decision 3 now says so. Tests: `packages/core/test/approval.test.ts` (the measure and the thresholds, table-driven) and `packages/api/test/approval-carry-over.test.ts` (criterion 4 through `editPage`, plus the accumulation, the disapproved case, the neutral no-op and the explicit-write override).

**Sprint 1, task 2 (ADR-078): a person can set the approval mark.** `setApproval` in `packages/api/src/operations.ts` is the one place the rule lives: it refuses an agent actor with `approval_person_only` (HTTP 403) naming the console and `cairn approve`, needs the version the person read, writes the mark with the content untouched, and records a revision whose note names the state (`Marked approved`, `Marked disapproved`, `Approval mark removed`) followed by the person's own note. `approvalVersion` is the version the person looked at, so later edits are measured against that body. Unmark clears all four fields. Surfaces: REST `POST /pages/:id/approval` (If-Match, body `approval`), and CLI `cairn approve`, `cairn disapprove`, `cairn unmark`. Since a bearer or OAuth token always names an agent, the CLI declares a person with the header `X-Cairn-Actor: person`, and refuses before any request when `CLAUDECODE` or `CAIRN_AGENT` is set; ADR-078 decision 2 now names that mechanism. Contract tests in `packages/api/test/approval.test.ts` and `packages/cli/test/approve-command.test.ts`; `docs/CLI.md` gains the section. The console buttons come with task 7.

**Sprint 1, task 1 (ADR-078): the page model carries an approval mark.** `Page` gains `approval` (approved, neutral, disapproved; neutral by default), `approvalAt`, `approvalVersion` and `approvalPrevious`; `PageInput` and `PageSnapshot` carry the same four as exact, optional values. Four new SQLite columns with a migration that leaves every existing page neutral. The store and the core page service keep all four whole when a write says nothing about them, the same rule `public` follows, so an ordinary edit cannot change the mark by accident; undelete restores them from the snapshot. Conformance test added (`packages/core/src/testing/document-store-conformance.ts`). No surface shows or sets the mark yet; that is tasks 2 to 8.

**Sprint 0 of `docs/PLAN.md`: ADR-078 to ADR-081 written, status proposed.** One per spec: knowledge approval (a person-only mark on the page, carry-over at 5 % / 200 characters, a 1.25 search boost, disapproved hidden by default, the mark travels with sync), the dropbox (an Inbox root collection, `POST /api/v1/drops`, drop tokens scoped to that one endpoint with a person-or-agent kind, no processing state), the mobile capture app (a PWA at `/m` with one service worker, which amends ADR-009's "no client-side application" rule for that file and the editor preview fetch), and the console for notes (preview column, callouts, toggles, task lists, tickable boxes, `CAIRN_TIME_ZONE`, template picker, owner home, delete and move from the console). Each answers its spec's open questions: the CLI tells a person from an agent by the `CLAUDECODE` and `CAIRN_AGENT` variables it already reads; a drop token carries a kind so the changes feed stays honest. Per the plan's gate 1, nothing else starts until the owner has read them.

**Added four specs and `docs/PLAN.md` for notes, knowledge approval, the dropbox and the mobile capture app.** The owner's direction after the product review: build what is lacking, with a plan. Design questions were settled with the owner first (`docs/DIRECTIONS.md`). `docs/specs/knowledge-approval.md`: a per-page mark, approved, neutral or disapproved, set only by a person, kept across small edits (at most 5 % and 200 characters changed against the approved version), reset to neutral with the old mark remembered otherwise; approved ranks 1.25 times higher in `searchPages`, disapproved is excluded unless `include_disapproved` is passed; a `/review` queue and a side-by-side diff view. `docs/specs/dropbox.md`: an "Inbox" root collection, `POST /api/v1/drops` for text and files, scoped and revocable drop tokens modelled on publish tokens, so a script or a phone gets in with auth but without the CLI or MCP; agents file a drop by moving it. `docs/specs/mobile-capture-app.md`: a progressive web app at `/m` in the same container, with a Web Share Target, an iOS Shortcut recipe and one service worker for an offline queue, which needs an ADR amending ADR-009. `docs/specs/console-for-notes.md`: a preview beside the editor, callouts, toggles and tickable checklists, Today with `CAIRN_TIME_ZONE`, a template picker, an owner home, and delete, move and table creation in the console. `docs/PLAN.md` orders them into five sprints, sprint 0 being the four ADRs (078 to 081) and a gate on the owner's yes. `docs/ROADMAP.md` gains a section for them and marks "Agent activity digest and approvals" and "Quick capture" as superseded. No code changed.
...
ADR-077: a new core port, `SynonymsStore` (`packages/core/src/ports/synonyms.ts`), backed by a new SQLite table, `synonyms`. A pair is curated under a collection (a top-level page, the same sense `packages/api/src/mcp/summary.ts`'s workspace summary already uses, not ADR-024's `Table`) but expanded workspace-wide at query time, since `SearchIndex.search()` has no collection scope. `resolveSynonyms()` (`packages/api/src/operations.ts`) tokenizes both sides of every pair with the same `tokenize()` function query terms go through, and attaches the other side as an alternate to every one of a matched side's token slots, only once all of that side's tokens already appear among the query's terms; a naive whole-string equality check was tried first and silently matched nothing for any hyphenated or multi-word term ("GLP-1" tokenizes to `glp` and `1`, never the single string `glp-1`), caught by a contract test and written up in `docs/LESSONS.md`. Wired on all four surfaces per hard rule 14: MCP `list_synonyms`/`add_synonym`/`remove_synonym`, REST `GET/POST/DELETE /collections/:id/synonyms`, CLI `cairn synonyms list/add/remove`, and a new console screen, `/p/:id/synonyms`, linked from a top-level page's own view, within ADR-009's plain-table console scope. The MCP server instructions gained a short clause ("Missing jargon? add_synonym, per collection"), fit under `FIXED_INSTRUCTIONS_CEILING` (1500 chars) by trimming a few words elsewhere in the same text; `skills/cairn/SKILL.md` gained the matching numbered step. `pnpm context-cost`: MCP total 2044 chars (~511 tokens) for instructions plus summary, tool list 26673 chars (31 tools); `README.md`'s context-cost table updated to the current, much larger workspace (126 pages, was documented against a stale 96-page snapshot) rather than just the delta from this change.

`pnpm eval` before and after: keyword recall@5 0.88, hybrid 0.97, unchanged, since the eval set (`eval/queries.yaml`) has no quoted phrases and no synonym pairs defined for this workspace yet, so both mechanisms run as a no-op against it; these are the same numbers already documented in `README.md`, confirming no regression. New tests: a `SynonymsStore` conformance suite (`packages/core/src/testing/synonyms-conformance.ts`), 1 MCP contract test covering list/add/remove and the cross-synonym search match (polled with `eventually`, per hard rule 11), 1 REST contract test, 1 CLI contract test. Full workspace suite green at 878 (up from 874 before this change's own new tests; 1 skipped, unrelated).
...
`createPageFromTemplate` (decision 3, `packages/api/src/operations.ts`) loads the template page (reusing `context.pages.get`'s own not-found error, since a template is just a page), substitutes the new title's placeholders into its body, and creates the result with `parentId` defaulting to the template's own parent when the caller sends none. `getOrCreateDailyNote` (decision 4, `templates.ts`) finds or creates the "Daily notes" collection eagerly (a note is about to be filed under it either way), but only looks up "Templates" without creating it, so asking for today's note before any template exists never leaves behind an empty "Templates" page nobody asked for; it looks for a child of "Daily notes" titled today's date, and if none exists, builds one from the "Daily note" template under "Templates" if there is one, empty otherwise. Wired on all three surfaces per hard rule 14: MCP `create_page_from_template`/`get_today_note`, REST `POST /pages/from-template`/`POST /pages/daily-note` (POST, not GET, since the first call of the day writes the note into existence), CLI `cairn new --template <id> --title T [--parent ID]`/`cairn today`. The MCP server instructions had only 39 characters of headroom left under `FIXED_INSTRUCTIONS_CEILING` this time (ADR-073's own build had 82), well short of what naming both new tool names would cost, so the added phrase is a short, vague pointer ("...or a page from a template") rather than the tool names themselves; `skills/cairn/SKILL.md` and the tool descriptions carry the specific commands instead.

New tests: 2 REST contract tests (`POST /pages/from-template` substituting both placeholders and defaulting `parent_id`; `POST /pages/daily-note` for the empty-body case, same-day idempotency, and the template-driven case in a fresh workspace), 2 MCP contract tests for the same two tools, 1 CLI contract test for `cairn new --template` and `cairn today`. One build-time lesson: `exactOptionalPropertyTypes` rejected an optional `parentId: undefined` passed straight through at both call sites, fixed with the conditional-spread idiom `routes.ts` already uses elsewhere (`docs/LESSONS.md`). Full workspace suite green at 867 (up from 863; 1 skipped, unrelated: the local-embedder test needs `CAIRN_TEST_MODEL`).

**Built ADR-074: publish and unpublish notify IndexNow, opt-in.** `docs/ROADMAP.md`'s "Tell search engines when publishing changes something" item named the gap: a publish or unpublish through `publishPage` (`packages/api/src/operations.ts`) waited for a search engine's own crawl schedule to notice. Off by default: a new `CAIRN_INDEXNOW_KEY` (`packages/api/src/config.ts`) opts in, following the same pattern `CAIRN_ATTACHMENTS_TO` and `CAIRN_BACKUP_TO` already use, unset means no key file is served and no outbound call is ever attempted. When set, `registerPublicWiki` (`packages/api/src/web/public.tsx`) serves the key itself at `GET /<key>.txt`, the proof `api.indexnow.org` checks before it accepts a submission; the console's sign-in middleware (`packages/api/src/web/console.tsx`) exempts that one path the same way it already exempts `/w`, `/sitemap.xml` and `/robots.txt`.

A new small module, `packages/api/src/indexnow.ts`, POSTs `{ host, key, keyLocation, urlList }` to `https://api.indexnow.org/indexnow`, timeout and `AbortController` mechanics matching `citations.ts`'s `safeFetchText`, but never throwing: a non-2xx response, a network error or a timeout is logged with `console.error` and swallowed. `publishPage` walks the published/unpublished page's subtree the same way `listStalePages` already does, a bounded cursor walk over `DocumentStore.listPages`, no new store method, collects every page's `<origin>/w/<id>`, and fires the submission after it has already decided its own return value, not awaited by the caller, so a slow or unreachable IndexNow can never delay or fail the publish response itself (ADR-074 decision 3, hard rule 5). Wired at both call sites that reach `publishPage`: REST's `POST /publish` and the console's `/p/:id/publish` form, each resolving the origin the same way `web/public.tsx`'s own `origin(c)` helper does, `CAIRN_PUBLIC_URL` if set, otherwise the request's own origin. No MCP tool: ADR-032 decision 5 already keeps publishing off MCP, and ADR-074 decision 4 confirms this is a side effect of an existing action, not a new capability, so hard rule 14 does not ask for a third surface here.

New tests in `packages/api/test/rest.test.ts`: a page with a child published with `CAIRN_INDEXNOW_KEY` set asserts the mocked IndexNow submission carries both URLs, the right host, key and `keyLocation`; the key file route serves the key as `text/plain`; and with the setting unset, no outbound call happens and the key file route answers the console's ordinary not-found. `docs/AGENT-OPERATE.md` gained a `CAIRN_INDEXNOW_KEY` entry, required by `packages/cli/test/agent-guides.test.ts`. Full workspace suite green at 863 (up from 860).

**Built ADR-073: stale-page reviews reach MCP, REST and the CLI, not just the console.** `docs/ROADMAP.md`'s "Stale-page reviews" item named the gap: ADR-028 built the console's `/freshness` screen, pages never verified first then oldest verified first, but that ordering was HTML for a person, with no way for an agent to ask for it short of walking every page itself. A new shared operation, `listStalePages` (`packages/api/src/operations.ts`), reproduces the screen's own order, built the same way its old `allPages` helper was: a bounded cursor walk over `DocumentStore.listPages` (same 5,000-page bound), sorted in memory, no new store method, column or index, so hard rule 2's conformance suite is untouched. Paginated with the same offset-cursor scheme `listChildren` already uses (`o:<offset>`, base64url), since there is no store-native cursor for an in-memory sort order. Exposed on all four surfaces per hard rule 14: MCP `list_stale_pages` (token-budgeted with `budgetList`, the same pattern `query_table` uses), REST `GET /pages/stale`, CLI `cairn stale [--limit N]`, and the console's `/freshness` screen itself, rewritten to call the shared operation (looping its cursor to keep showing every page in one table, unchanged from before) instead of its own inline filter-and-sort, so all four now agree by construction. The console's summary line and "Collection" column needed no second query: `listStalePages` returns `never_verified_count` and the oldest verified page's `verified_at` alongside each page's `id`/`title`/`parent_id`/`verified_at`/`updated_at`, and since the screen already showed every page rather than a filtered "stale" subset, ancestor lookups for the Collection column stay correct using only the returned items (`ancestorsOf` was generalized to accept any `{ parentId }` shape, not just the full `Page` type). No pending-write or approval state was built: ADR-073 confirms with the owner that "drafted by an agent for the owner to confirm" needs the separate, larger "Agent activity digest and approvals" roadmap item, not this one; a re-check today is an ordinary write with `verified: true`, reviewed the way any other agent write already is. The MCP server instructions and `skills/cairn/SKILL.md` each gained one sentence pointing an agent at `list_stale_pages`/`cairn stale`, kept under `FIXED_INSTRUCTIONS_CEILING` by folding the new sentence into the existing `verified: true` line rather than adding a paragraph. `pnpm context-cost`: MCP tool list 25 to 26 tools, 22211 to 22954 chars (~5553 to ~5739 tokens); instructions plus summary 1964 to 2007 chars; skill body 5095 to 5302 chars. New tests: 1 REST contract test (ordering, pagination, `never_verified_count`/`oldest_verified_at`), 1 MCP contract test (same, via `list_stale_pages`), 1 CLI contract test (`cairn stale`), plus `list_stale_pages` added to the MCP tools-list assertion. Full workspace suite green at 859 (up from 856).

**Accepted ADR-072: the vector margin miss on q17 is a limitation, not a bug.** Baseline `pnpm eval` before any change: keyword recall@5 0.88, hybrid 0.97 (31 of 32), q17 ("which peptide makes you really hungry") the only miss in either mode, so hybrid was not the cause. Hybrid already spends precision to buy that recall elsewhere: it answers only 4 of 9 no-answer queries correctly against keyword's 7, the same margin governing both. Rather than tune `DEFAULT_MARGIN` against one query (the exact overfitting `docs/ROADMAP.md` already warned against), the actual similarity curve behind q17 was logged directly (temporary instrumentation in `vectorRanking`, reverted, confirmed by `git status`): the top 250 nearest chunks decay smoothly from 0.7246 to 0.59 with no elbow anywhere, because the workspace is one coherent topic and "appetite" is graded across many pages rather than split into relevant and irrelevant clusters. No baseline statistic tried against that curve, median, trimmed mean, a shifted rank window, can tell this case apart from a genuine false positive without the same risk. No code changed; the roadmap item is closed as investigated and accepted, per hard rule 7's before/after requirement, with a content edit to pg_ghrp-6's own page named as the safe alternative if this is ever revisited.

**Built ADR-070: a sync conflicts list in the console.** `DocumentStore.listRecentRevisions` (`packages/core/src/ports/document-store.ts`) gained an optional `syncConflict?: boolean` filter, matching revisions whose `note` contains the literal substring `"Sync conflict:"`; combines with the existing `actorKind` filter as AND. `adapter-sqlite` implements it as a parameterized `note LIKE '%Sync conflict:%'` clause alongside the existing `actor_kind` condition, not FTS (ADR-021 is for search relevance, not an exact substring match on a column already scanned). Covered by the shared conformance suite (hard rule 2), one new case seeding ordinary and sync-conflict revisions across both actor kinds. The console's `/changes` route (`packages/api/src/web/console.tsx`) gained a third toolbar chip, "Sync conflicts", toggling `?conflict=1`, combinable with `?who=`; the pagination link now carries both. Cross-reference comments added at `packages/cli/src/sync.ts`'s `note()` and at the new SQL condition, per ADR-070 consequence 2, so a change to the marker string is visible from both sides.

**Accepted ADR-071: ADR-070's "three note shapes" was wrong, and checked that the filter still catches everything.** The build surfaced that only two of the note's shapes carry the `Sync conflict:` marker, not three: a delete (plain, or `verifyDeletes`'s unprovable-deletion recreate) never does. Tracing `plan()` shows this is not a gap: `conflict` is only ever `true` in the branch where both sides changed content, and that branch's `op` is always `"put"`, never `"delete"` — a delete action's `conflict` is `false` by construction, every time, because ADR-030 defines a conflict as both sides changing a value differently, which a one-sided deletion against an unchanged other never is. The unprovable-deletion case is a separate, already-visible thing (a `warning:` line, not a `conflict:` one). Nothing was missed; `/changes?conflict=1` already catches every revision where a real conflict happened. No code changed.

**Accepted ADR-070: a sync conflicts list in the console, derived from history.** Closes the gap `docs/ROADMAP.md` named, left out of ADR-030: reviewing where a sync conflict took the newer edit, and reaching the version it replaced, from the console rather than only the `cairn sync` terminal output. Derives the list from the `"Sync conflict:"` marker already written into every conflict's change note, no new storage; a standing list across all history, not a one-off report of the last sync run. Spec in `docs/specs/sync-conflicts-console.md`: a new `syncConflict?: boolean` filter on `listRecentRevisions`, and a third toolbar chip on the console's existing `/changes` page. Not yet built; both decisions made with the owner via AskUserQuestion before writing the ADR.
