...
4. **Find out which Cairn first.** Run `cairn instances`. If it lists instances, use their names with `--instance <name>`, or its short form `-i <name>`. If not, ask for the address, and remember that without `CAIRN_URL` or `--instance` the CLI talks to `http://localhost:8787`.
...
| `CAIRN_AUTH_SECRET_PREVIOUS` | none | The old signing secret during a rotation (section 10) |
...
| `CAIRN_INDEXNOW_KEY` | none | Opt-in (ADR-074): when set, publishing or unpublishing a page notifies `api.indexnow.org` about every page in the affected subtree, so a participating search engine (Bing, Yandex and others) can recrawl sooner instead of waiting for its own schedule. Unset, the default, means the feature is off entirely: no key file is served and no outbound call is ever made. Pick any opaque string IndexNow accepts, such as `openssl rand -hex 16`. Cairn then serves it at `/<key>.txt`, the proof IndexNow checks before it accepts a submission. A slow or unreachable IndexNow never delays or fails a publish: the notification runs after the write, logged on failure, never retried |
...
| `CAIRN_BACKUP_TO` | a `backups` folder beside the database | Where backups go (ADR-049, ADR-050). Each one is a complete, self-contained copy of the database, made with `VACUUM INTO`, read back before it is accepted, and named for the time it was taken. Takes a folder path, `abs://<account>@<container>/<prefix>` for Azure Blob Storage, or `s3://<bucket>/<prefix>` for S3 or anything that speaks S3. `off` turns backups off entirely. On a platform whose disk does not outlive the container, a folder means the backups die with it, and Cairn warns at startup when it sees that |
| `CAIRN_BACKUP_REGION` | `AWS_REGION`, then `AWS_DEFAULT_REGION` | Only for an `s3://` destination, which cannot be signed for without a region. Cairn refuses to start rather than guess one |
| `CAIRN_BACKUP_ENDPOINT` | none | Only for `s3://`: the address of a store that speaks S3 but is not AWS, such as MinIO. Buckets are addressed as a path under it |
| `CAIRN_ATTACHMENTS_TO` | `off` | Where uploaded files go (ADR-064): `abs://<account>@<container>/<prefix>` for Azure Blob Storage, or `s3://<bucket>/<prefix>` for S3 or anything that speaks S3. `off`, the default, turns attachments off entirely; `create_attachment` and the REST/CLI equivalents then fail with an error naming this setting. A plain folder is not accepted here: an attachment needs a signed URL a browser or an agent can upload straight to, and a local folder cannot issue one. Reuses `CAIRN_BACKUP_REGION` and `CAIRN_BACKUP_ENDPOINT` above for `s3://`, rather than its own copies of the same two settings |
| `CAIRN_BACKUP_AFTER_HOURS` | `3` (the Azure deploy script defaults to `0.5`, ADR-062) | After a write, back up if the newest backup is older than this. Activity is the trigger rather than a timer, because a timer never fires in a container that has scaled to zero. Azure's deploy default is tighter because this path does not go through Litestream and so is unaffected by issue #1515 (ADR-061) |
...
| `CAIRN_RESTORE_TRIES` | `12` | How many times a restore that failed for a reason that could pass on the next try, such as a network error or a role Azure has granted but not yet propagated, is attempted before Cairn gives up on the replica (ADR-051). Damage is never retried, however high this is set |
| `CAIRN_RESTORE_WAIT_MS` | `10000` | How long to wait between those attempts. The default gives a first deploy two minutes for its identity to be granted access to storage |
| `CAIRN_APP_DIR` | `/app` | Where the container's own bundled files are. Only the image's start script reads it, and only the test suite ever sets it |
...
1. `cairn status` (with `--instance` or `CAIRN_URL`) is the fastest check: one screen for whether the instance answers, sign-in, sync, embeddings, the scheduled job and the session hook, each not-ok line carrying the command that fixes it (ADR-053). `cairn status --json` for a script.
2. `curl -s <address>/health` prints `"status":"ok"`.
3. On a server with OAuth, `curl -s -o /dev/null -w '%{http_code}\n' -X POST <address>/mcp` prints `401`: it refuses anyone not signed in.
4. `cairn overview` (with `--instance` or `CAIRN_URL`) lists collections and tables. If it says to sign in, the person runs `cairn login --instance <name>` or `CAIRN_URL=<address> cairn login`; it opens their browser.
5. `cairn changes` shows recent writes, newest first, and who made them.
6. The logs, from section 1, show no restarts or errors.
...
2. **Finish and commit your own writes before you sync.** A dry run only tells the truth about a side that already holds everything you meant to write to it: finish and save (in a code repo, `git commit`; in Cairn itself, whatever write you were mid-way through) before you run one, not after. A sync started while you still have pending writes describes a plan for state that is about to change out from under it.
3. **If you just fixed a bug in `cairn sync` itself, check which `cairn` you are about to run before trusting its dry run.** `which cairn` can resolve to a globally npm-installed build that predates a fix you made minutes ago in this repository; `cairn --help` and a page count do not tell you that, and a stale binary's dry run reads as clean right up until it proposes the exact deletion your fix exists to stop. Check `readlink -f "$(which cairn)"` and its install date, or just run the freshly built CLI directly (`node packages/cli/dist/bin.js sync ...` after `pnpm build`) instead of trusting PATH, whenever the fix you are about to exercise landed in this same session. Found 2026-09-18: see `docs/LESSONS.md`.
4. **Sync, always dry run first, no exceptions.** Run `cairn sync --dry-run` and read its output before running `cairn sync` for real, even when the person already told you to sync, even mid-task, even when you are in a hurry to finish something else. "sync" from the person is permission to sync, not permission to skip the dry run. With instances registered, it syncs them all through the first that answers. `cairn sync <a> <b>` syncs one pair.
   Read every `warning:` line in the dry run before running for real: ADR-060 means `cairn sync` no longer deletes a page it cannot prove was deleted (it recreates it instead and warns), but a warning still means something looked wrong and needs a human's eyes, not a rubber stamp.
   If you have any reason to suspect a page went missing outside of a normal delete (the person mentions something is gone, a page count looks wrong, a prior sync or deploy misbehaved), do not run `cairn sync` for real until you and the person understand why, even after a clean dry run. Say what you suspect and ask first. `cairn sync` deleting the survivor of a real data loss, because it looks to sync like the other side "changed" that record to absent, is exactly the failure ADR-060 exists to explain: see it and `docs/LESSONS.md` (2026-09-18) before touching sync again after any anomaly.
5. **On a schedule:** `cairn sync install --every 4h`, after asking; it installs a launchd, systemd or Task Scheduler job. `cairn sync uninstall` removes it. Four hours is the default and is six wakes a day. Syncing more often than `CAIRN_IDLE_MINUTES` keeps a Cairn on Azure awake, which costs more, so shorten it only if the person asks for that after hearing the cost. When they want a change across now, run `cairn sync` once instead.
6. **Starting the day:** `cairn start` starts the first instance if it is down and syncs.
7. **Reading a report.** `merged:` means both sides changed a record in different parts and both edits were kept. `conflict:` means both changed the same part, and the newer edit won; the other is in that record's history, and `cairn history <page-id>` shows it. Tell the person about every conflict.

## 6. Back up and restore

1. **Cairn backs itself up.** After a write, if the newest backup is more than `CAIRN_BACKUP_AFTER_HOURS` old, and again on every tidy shutdown, it writes a complete copy of the database to wherever `CAIRN_BACKUP_TO` points and reads it back before keeping it (ADR-049). Each one is a plain SQLite file any `sqlite3` can open. The startup log says how many there are and how old the newest is.
...
3. **Azure:** Litestream streams every change to the storage account, so a new container starts from a copy seconds old. That is a replica, not a backup: it copies SQLite's pages, so damage reaches it too, which is what happened on 2026-09-15 (ADR-046). The backups in point 1 are the independent copy, and the template sends them to a second blob container, `cairn-backups`, reached through the app's managed identity (ADR-050). List them with `az storage blob list --account-name <account> --container-name cairn-backups --auth-mode login -o table`. Both still live in one storage account, so also suggest a `cairn export` the person keeps themselves.
4. **Their own server:** the `data` folder, with the machine's own backups, and optionally `CAIRN_REPLICA_URL` for a copy off the machine. Here `CAIRN_BACKUP_TO` defaults to a folder on the mounted volume, so the backups survive the container; it also takes an `s3://` address.
5. **Cairn recovers itself first, so check the log before restoring anything by hand.** When there is a replica, every start climbs down a ladder (ADR-051): the database already there if it passes its integrity check, then a plain restore from the replica, then the newest point in the replica that restores and reads back, then the newest backup that does. It says in the log which rung it landed on. A database it would not open is renamed to `<database>.broken-<timestamp>` and left there for good; nothing removes it but the person. If the log says it recovered from a backup, say so plainly: work written after that backup is not in the database, and once Cairn starts, Litestream replaces the replica's history with what it now holds.
6. **Restore from a backup by hand,** when the ladder could not: stop Cairn, move the damaged database aside, copy the backup into its place under the name `CAIRN_DB` gives, and start. Never copy a backup over a database a running Cairn has open.
7. **Restore from an export:** `cairn import <folder> --dry-run`, then without `--dry-run`, into any Cairn. It keeps ids, and running it twice changes nothing.
8. **Undo one change:** every write is a revision (ADR-008). `cairn history <page-id>` lists them, and the person restores one in the console.
...
The owner's judgement of a page (ADR-078): approved, neutral or disapproved. Commands in `docs/CLI.md`, "Marking what you have checked".

1. **You cannot approve.** Only a person sets the mark. `cairn approve`, `cairn disapprove` and `cairn unmark` refuse to run when `CLAUDECODE` or `CAIRN_AGENT` is set, and MCP has no tool for it. Tell the person to use the buttons on the page in the console, or to run the command themselves.
2. **The review queue** is the console's `/review`: pages an edit changed since they were approved first, then pages an agent wrote and nobody has marked, newest first, then the rest by how many pages link to them. The home page counts what is waiting. When the person asks what to look at, or after a day of agent writes, point them there.
3. **What agents do with it.** Search ranks approved pages higher and leaves disapproved ones out unless asked (`cairn search --include-disapproved`, MCP `include_disapproved`, REST `?include_disapproved=1`). Every read shows the mark, and on a disapproved page, or one approved and changed since, one notice line before the body.
4. **A large edit resets it.** A change of at most 5 % and 200 characters with the title unchanged keeps the mark; anything more sets it back to neutral with a trace, and the page joins the queue. The person can keep the mark through a large edit with the "Keep the mark" box on the console's edit form.
5. **It travels.** Unlike publishing, the mark goes with `cairn sync`, `cairn export` and `cairn import`. On the other Cairn it points at the copy written there, so "Changes since approval" works on both.

## 9. The Inbox and drop tokens

The dropbox (ADR-079): the person throws text and files at Cairn from a phone, a shell or a script, and an agent files them later. Commands in `docs/CLI.md`, "Throw things in for later".

1. **The Inbox** is a root collection named `Inbox`, created the first time something is dropped. A drop is an ordinary page under it, tagged `drop`, with its files linked from the body. There is no processing state: a drop is unprocessed while it sits there.
2. **File what waits.** The workspace summary says `Inbox: N drops waiting to be filed` and `cairn drops` lists them. For each: read it, search for where it belongs, write what it says there (append or create, with a change note naming the drop), then move the drop under that page (`cairn move <id> --parent <page> --version V`) or delete it once fully absorbed. Never leave a used drop in the Inbox, and never file one the person has not asked you to touch if they said they would do it themselves.
3. **A drop token** lets a phone's share sheet or a script drop without the CLI or OAuth. It is accepted only on `POST /api/v1/drops`; anywhere else it gets `403 drop_token_scope`. It is stored as a SHA-256 hash and shown once, at creation: `cairn drop-token create <name> [--kind person|agent]`, `list`, `revoke <id>`, or the console's `/settings/drop-tokens`. Never through MCP. If the person lost the value, revoke it and make another; nothing can show it again.
4. **The kind is recorded on every drop:** a `person` token writes as the person, an `agent` token as an agent, each labelled `token "<name>"` in the history and the review queue. Choose `agent` for anything a script holds.
5. **Files need `CAIRN_ATTACHMENTS_TO`.** With it off, a drop with files fails with `attachments_off` naming the setting; text-only drops always work. Limits: 10 files of 25 MB each, 64 KB of text, two drops in flight per server. HEIC from an iPhone arrives as a file link, not a thumbnail.
...
1. **Start with `cairn status`.** It reports instance, sign-in, sync, embeddings, job and hook in one screen, each not-ok line carrying the command that fixes it, so you rarely need to work out which of those five is broken by hand.
2. **Read the message.** Cairn's errors name the setting, command or step that fixes them. Do what it says before anything else.
3. **Then the person's guide:** "When something goes wrong" in `docs/DEPLOY-AZURE.md` or `docs/DEPLOY-DOCKER.md`, and `docs/LESSONS.md`.
4. **Common cases:**
   ...
   6. The address answers 502, 503 or 504, or nothing at all, for more than a couple of minutes: the request is not reaching Cairn, so the reason is in the container's log and nowhere else. Read it: on Azure, `az containerapp logs show -n <app> -g <group> --tail 50`; with Docker, `docker compose logs --tail 50`. Do not trust the platform's own health here, because an Azure container app reports "Running" and "Healthy" whether or not the program inside it started (`docs/LESSONS.md`, 2026-09-16). If the log says the replica cannot be read back, Cairn has already tried everything it can (ADR-051): the local database, a plain restore, the newest point in the replica that reads back, then the newest backup. Stopping means all four failed, so it stops rather than serve a database it cannot verify (ADR-046). The log lists what the replica holds and the exact commands for the three ways out. One case is not a failure of the data: if the log says the replica could not be reached, that is usually a network rule or the app's identity having lost its role on the storage account, and Cairn deliberately does not fall back to a backup there, because the replica is probably fine and an older database would then be streamed over it. Never redeploy from empty until you are certain no other Cairn holds a newer copy, because the first write overwrites the replica.
5. **If the fix is not in the docs,** tell the person what you saw, and do not change code or cloud resources unasked. Afterwards, suggest a `docs/LESSONS.md` entry, so the next agent finds it.
