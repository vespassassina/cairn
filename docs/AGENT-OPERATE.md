...
2. **Sync, always dry run first, no exceptions.** Run `cairn sync --dry-run` and read its output before running `cairn sync` for real, even when the person already told you to sync, even mid-task, even when you are in a hurry to finish something else. "sync" from the person is permission to sync, not permission to skip the dry run. With instances registered, it syncs them all through the first that answers. `cairn sync <a> <b>` syncs one pair.
