...
The two bugs from the review landed first, in commit `949ff93` (the entries below). The rest is design and planning, no code. Four choices were put to the owner with a recommended default each and the owner took all four: agents see every page with its mark, approved ranks higher, disapproved is hidden unless asked for; the dropbox is an "Inbox" root collection with one page per drop, and agents get in without the CLI or MCP through scoped drop tokens; the mobile app is a progressive web app served by the same container at `/m`; the preview sits beside the editor's textarea. Landed as four specs, `docs/specs/knowledge-approval.md`, `dropbox.md`, `mobile-capture-app.md` and `console-for-notes.md`, and `docs/PLAN.md` with five epics in five sprints, ADR-078 to ADR-081 named for sprint 0. A roadmap section lists the items as planned. The owner answered "yes" to the specs and the plan the same day, so sprint 0 ran: ADR-078 to ADR-081, status proposed, indexed in `docs/decisions/README.md`. Gate 1 of the plan now waits on the owner reading them; no code before that.
...
### "so, also since this is an agentic tool, when the agent is running the sync (after committing documents it should) the agent first checks the dry run then commits"

A standing operating principle, not specific to this incident: an agent using Cairn should finish and save its own pending writes first (a `git commit` in a code repo, a pending Cairn write), only then run `cairn sync --dry-run`, and only apply the real sync after reading that plan. Added to `docs/AGENT-OPERATE.md` §5 as a new point 2, ahead of the existing dry-run-first rule (ADR-060), so the order is explicit: finish, dry run, then sync for real.
