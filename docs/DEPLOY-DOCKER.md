...
1. **Cairn's own backups.** Cairn writes a complete copy of the database into `data/backups` after a write when the newest copy is more than three hours old, and again every time it shuts down tidily. Each copy is checked by reading it back before it is kept, and each is a plain SQLite file you can open with `sqlite3` (ADR-049). It keeps two days' worth, and never fewer than three however old they are. `CAIRN_BACKUP_TO`, `CAIRN_BACKUP_AFTER_HOURS`, `CAIRN_BACKUP_KEEP_DAYS` and `CAIRN_BACKUP_KEEP_AT_LEAST` change this, and `CAIRN_BACKUP_TO=off` turns it off. `CAIRN_BACKUP_TO` also takes an `s3://bucket/prefix` address if you would rather keep backups off the machine entirely. Because `data` is a mounted volume, these survive the container. To use one: stop Cairn, move the damaged database aside, copy the backup in under the same name, start.
