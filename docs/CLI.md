...
The approval mark is the owner's judgement of a page (ADR-078), separate from `verified`, which any agent can set after re-checking facts. Only a person sets it: the three commands refuse to run when `CLAUDECODE` or `CAIRN_AGENT` is set, the same signals the CLI uses to name an agent in the history, and tell the agent to ask you instead. Each mark is a revision whose note names the new state, with your `--note` after it. A small later edit keeps the mark; a large one, or a title change, drops it back to neutral and the console's review queue shows the page as changed since approval. The mark travels with `cairn sync`, `cairn export` and `cairn import`, unlike publication: on the other Cairn it points at the copy that was written there, so "Changes since approval" works on both.
...
4. **They offer to sync themselves.** When you register the second Cairn, `cairn instances add` asks whether to keep them in sync every four hours with no terminal open. Yes installs it; anything else installs nothing and prints the command for later. You are asked once, never again once a job exists, and never when input is piped or you are running from a script, where it prints the command instead (ADR-052).
5. **`cairn sync install --every 4h`** is that same job, installed yourself: a launchd agent on macOS, a systemd user timer on Linux, a scheduled task on Windows. Add `--dry-run` to see what it would install. `cairn sync uninstall` removes it. It needs the installed `cairn` (not `pnpm cairn`), and it uses your sign-ins, never a token. Output goes to `logs/sync.log` beside them.
6. **How often.** Every sync wakes a Cairn on Azure that has gone to sleep, and it then stays up about 30 minutes. Four hours, the default, is six wakes a day and leaves it asleep about three hours in four. Every hour leaves it awake about half the time; every 30 minutes or less keeps it always on, and costs the same as one that never sleeps. When you want a change across right now, run `cairn sync` rather than shortening the schedule.
7. **Limits.** On Windows the task cannot carry `XDG_CONFIG_HOME` or `CAIRN_CREDENTIALS`, keeps no log and does not run at login. On Linux the timer runs only while you are logged in, unless you run `loginctl enable-linger`. Claude Code and claude.ai keep the one address they were set up with; only the CLI picks among instances.
