...
2. **Cause.** FTS5's tokenizer (and this codebase's own `tokenize()` in `packages/core/src/search/terms.ts`) splits on any non-letter/non-digit character, so `"GLP-1"` becomes two separate query terms, `"glp"` and `"1"`, never the single string `"glp-1"`. The synonym store, meanwhile, saved a pair's `term`/`synonym` folded but not tokenized (`foldTerm`, which lowercases and strips diacritics but keeps hyphens), so the stored key was the single string `"glp-1"`. `resolveSynonyms()` checked `terms.has(pair.term)` against the tokenized query terms, and `"glp-1"` was never a member of a set containing `"glp"` and `"1"` — the two sides could never be equal for any term with internal punctuation or spaces. A single quick manual smoke test (calling `searchPages()` directly) initially looked fine because it happened to only exercise the case where a page's own text repeated the exact query text, not the actual synonym-only page; the gap only showed up once the contract test asserted the true cross-synonym match.
3. **Fix.** Rewrote `resolveSynonyms()` to tokenize each side of a stored pair with the same `tokenize()` function query terms already go through, and to attach the *other* side as an alternate to every one of that side's own token slots, but only when all of those tokens are already present among the query's terms. A page containing only the spelled-out synonym now satisfies every term slot `"GLP-1"` would have required, and single-word pairs (the common case, e.g. an acronym with no punctuation) work exactly as before. Verified with a contract test in `packages/api/test/mcp.test.ts` polling via `eventually()` per hard rule 11 (search is eventually consistent).
4. **Lesson.** When a synonym, alias, or any other per-term lookup key is built from user-typed text, fold it through the *exact* tokenizer the search path itself uses, not just case/diacritic normalization, and test it with a term that contains punctuation or spaces before considering the feature done. Domain jargon in a wiki about compounds and dosages is disproportionately hyphenated and multi-word ("GLP-1", "BPC-157", "growth hormone releasing peptide"), so this is not an edge case for this workspace, it is the typical case.
...
2. **Cause.** ADR-068 was written from jSquash's own package list without checking each name against the registry. `npm view @jsquash/gif` and a direct `GET https://registry.npmjs.org/@jsquash/gif` both 404; jamsinclair/jSquash's own `packages/` directory (checked via the GitHub API) has never had a `gif` package: avif, jpeg, jxl, oxipng, png, qoi, resize, webp, and nothing else. A GIF decoder does exist, but only republished under an unrelated scope (`@discourse/gif`, a Discourse fork), which is not what ADR-068 named or reviewed.
3. **Fix.** Not fixed. `image/gif` stays one of the four raster content types ADR-064 decision 5 accepts, but `attachments-thumbnail.ts`'s `makeThumbnail` has no decoder wired up for it and throws `ThumbnailUnavailableError`, which `attachments.ts` already treats the same as any other generation failure: `thumbnailKey` stays null, nothing throws into the caller. `docs/ROADMAP.md`'s attachments row is marked "GIF thumbnails blocked" pending a developer decision: pick a real package (possibly `@discourse/gif`, unreviewed) or amend ADR-068.
4. **Lesson.** An ADR that names a specific package should be checked against the registry before it is accepted, not just against the vendor's README or package list from memory. A library's own docs can describe packages that were planned, renamed, or never actually published; `npm view <name>` or a registry `GET` takes one command and would have caught this before decision 6 was ever scoped as "four raster types."

### jSquash's codecs need a Node-specific `fetch` shim for their own `.wasm` files, and `@jsquash/resize`'s `ImageData` needs Node 22/24's own polyfill path, not a hand-rolled one

1. **What happened.** The first attempt at calling `@jsquash/png`'s `decode`/`encode` directly from a Node script failed with `TypeError: fetch failed`, `Error: not implemented... yet...`, from inside the package's own generated wasm-bindgen glue code.
2. **Cause.** Every jSquash codec, both the wasm-bindgen ones (png) and the Emscripten ones (jpeg, webp), calls a bare `fetch(new URL('its-own.wasm', import.meta.url))` to load its own wasm binary when no module is passed to its `init()`. That is right for a browser or a bundler; Node's built-in `fetch` (undici) refuses a `file://` URL outright. jSquash's own README calls Node support "experimental" for exactly this reason, and its one Node example (`examples/with-node`) works around it by manually compiling each `.wasm` file and calling `init()` with the result, which is workable but means re-deriving each package's wasm file path and calling four separate `init()` functions by hand.
3. **Fix.** `attachments-thumbnail.ts` instead patches `globalThis.fetch` once, only for `file://` URLs (every other request passes straight through to the original `fetch`): read the file with `node:fs/promises` and hand back a `Response` wrapping its bytes. Every codec's default wasm-loading path then just works, no per-package `init()` calls needed. Separately, `@jsquash/png`'s own compiled output already defines a global `ImageData` polyfill when it detects it is running in Node (`isRunningInNode` in its `codec/pkg/squoosh_png.js`), so `attachments-thumbnail.ts` does not need to define its own; an early version of this code did, and it worked, but was redundant with what the package already does the first time any jSquash codec loads.
4. **Lesson.** When a WASM-in-the-browser library claims "experimental" Node support, check what its own wasm-loading call actually does (usually a bare `fetch` against a relative URL) before reaching for the library's documented Node workaround; patching the one shared primitive it depends on can cover every one of its packages at once, and is less code than re-deriving each package's init call.
...
1. **What happened.** ADR-062's source-built Litestream (PR #1514's fix for issue #1515) was deployed to Azure and, per its own stated verification standard, watched live: a fresh L9 snapshot appeared at startup and compaction advanced through L0, L1 and L2 within five minutes, which read as success. The owner then reported the Azure console showing 2 collections instead of 3. `cairn --instance azure overview` confirmed it: 109 pages against the laptop's 122, missing the entire "Home Assistant & Homelab" collection and ten pages under "Projects".
2. **Cause.** The verification watched the wrong thing. Rereading the container's own startup log (not checked at deploy time) showed the restore had already failed and recovered before that first snapshot was written: `the replica would not restore: ... nonsequential page numbers in snapshot transaction: 1459,1500`, followed by `restored the replica as it stood at 2026-09-18T09:32:26Z ... 9 later points were unreadable and left behind`. The source-built Litestream's snapshot writer produced a malformed L9 snapshot; Cairn's own restore integrity check (ADR-046) correctly refused to start on it and stepped backward to the last point that passed, discarding nine real replication points on the way. Watching compaction advance afterward proved the fix for #1515 worked; it said nothing about whether the restore that preceded it had already thrown data away.
3. **Fix.** Reverted the Dockerfile's Litestream stage to the pinned, checksummed 0.5.17 release (ADR-063). Recovered the missing pages with `cairn export --instance laptop` then `cairn import --instance azure` (dry run first: 13 created, 1 updated, 108 unchanged, no deletions). `CAIRN_BACKUP_AFTER_HOURS=0.5` was left in place; it never touches Litestream and was not part of the failure.
4. **Lesson.** A near-miss almost became a second, worse mistake here: `cairn sync --instance azure --dry-run` was tried first, out of habit from the previous incident's recovery, and it proposed deleting the same 13 pages from the laptop copy, the only good copy left, because sync's bookkeeping read Azure's post-restore state as an intentional deletion made after the last recorded sync rather than as data loss. Always dry-run a recovery command and read what it proposes before running it for real, especially the "obvious" command that worked last time: the two incidents looked identical from the outside (Azure short some pages) but had opposite correct recoveries (recreate on Azure vs. propagate a delete to laptop), and only the dry run told them apart. Separately: verifying a replica fix by watching compaction advance afterward is not enough; read the startup log for a restore fallback first, since a restore can silently discard data and still leave a perfectly healthy-looking replica behind it.

### The undelete-ordering bug flagged as "latent" in an earlier fix materialized: undelete failed with `not_deleted` on CI

1. **What happened.** Pushing the Litestream mitigation (ADR-062) triggered CI, which failed `rest.test.ts`'s undelete test on both `ubuntu-24.04-arm` and `macos-latest`: `POST /pages/:id/undelete` returned 422 `not_deleted` for a page that had just been deleted and confirmed present in `GET /pages/deleted`. Passed every time locally.
2. **Cause.** Exactly the bug an earlier fix (see "`list_deleted_pages` flaked on CI" above) already named and left unfixed on purpose: `listRevisions`'s `ORDER BY created_at DESC, version DESC` breaks same-millisecond ties by comparing `version`, a random UUID, so on about half of ties it returns the older revision as "the latest." `undelete` (`packages/core/src/services/pages.ts`) calls `listRevisions(..., { limit: 1 })` and trusts `items[0]` to be the deletion revision; on a fast CI runner, the create and the delete can land in the same millisecond, and when the tie broke the wrong way, `items[0].deleted` was `false`, so `undelete` threw `PageNotDeletedError`. The earlier fix explicitly listed `listRevisions` and `listRecentRevisions` as carrying this same risk and left them alone because nothing had surfaced a failure from them yet; this is that failure.
3. **Fix.** Applied the same rowid tie-break to `listRevisions`, `listRecentRevisions`, and `listDeletedPages`'s own list ordering (`packages/adapter-sqlite/src/document-store.ts`), and moved the pagination cursor for all three from `[createdAt, version]` to `[createdAt, rowid]` so the cursor's keyset bound stays consistent with the new ordering. `pnpm test` passes; the fix is structural (rowid is monotonic by construction), not timing-dependent, so it cannot flake the way the old comparison did.
4. **Lesson.** "Worth revisiting" in a lessons entry is a debt, not a note. When a fix explains that a bug pattern exists in sibling code paths and says so in writing, either fix every sibling in the same change or open a tracked follow-up immediately, because "nothing has surfaced a failure from it yet" is exactly the condition that changes without warning the next time something runs a little faster.

### The Litestream replica silently stopped advancing, twice more the same day, and it is a confirmed upstream bug (#1515) with no fix released

1. **What happened.** After ADR-060 shipped, redeployed, and a clean sync, the owner reported the same Home Assistant pages missing from Azure again: "i again have lost the online docs on home assistant collection." `cairn sync`'s state file showed it had not run since before the redeploy, ruling out the tool ADR-060 had just fixed. Recovering the pages with `cairn sync` a second time, then watching the replica by hand, caught the bug happening a third time within the hour: the recovery's own writes were deleted from the replica about seven minutes after landing, while the container was still up and serving them correctly, well before any restart. A restart roughly 45 minutes later (most likely the 30-minute idle timeout) then exposed the loss the same way as before.
2. **Cause.** Listing the replica's Azure Blob container end to end found no level-1 or level-2 compaction files between `2026-09-15T20:40:00Z` and `2026-09-18T08:18:36Z`, a 59.5-hour gap, despite the app being demonstrably active for most of it. Watching the same thing happen live pinned the mechanism exactly: the container's log kept reporting `max_l1_txid=0000000000000080`, the stale high-water mark from before the 09-15 stall, unchanged for 17 minutes while fresh writes kept arriving at level 0. L0 retention deletes a file once its txid is at or below that number, so it deleted the recovery's writes without them ever being compacted into level 1. This is [Litestream issue #1515](https://github.com/benbjohnson/litestream/issues/1515), confirmed and reproducible upstream: the follower's gap-bridging logic only checks compaction levels 1-8, never level 9 (the daily snapshot), so when it hits a gap it cannot bridge it silently stops advancing with no error and no log line, and a restart resumes at the same stuck position instead of clearing it. Explicitly affects 0.5.14, 0.5.16, and 0.5.17, the version Cairn runs; no released fix exists, and the two candidate fixes ([#1516](https://github.com/benbjohnson/litestream/pull/1516), [#1514](https://github.com/benbjohnson/litestream/pull/1514)) are both still open.
3. **Fix.** No code fix: the bug is in Litestream, not in Cairn or in `cairn sync` (which behaved correctly throughout, converting the unprovable "deletes" into recreates per ADR-060). ADR-061 records the confirmed cause and lays out the options (wait for an upstream release, build against an unreleased fix commit, add a replica-lag watchdog, or lean harder on Cairn's own independent backups which don't go through this code path) as the owner's call, since each is an infrastructure or operational decision.
4. **Lesson.** A stall like this leaves no trace in the tool's own logs, by design of the bug: don't trust "no errors in the log" as evidence a replica is healthy. Check the replica's actual advancement directly (fresh LTX file timestamps, not just that a write's HTTP call returned success) before calling any Azure recovery done, and don't redeploy, restart, or let the container idle out soon after a manual recovery until that check has passed. When a suspicious pattern reproduces once, try to reproduce it again on purpose, under watch, immediately: it is what turned "probably a stalled replica" into a named, confirmed, three-line upstream root cause with a linked issue number, instead of a second guess.

### `cairn sync` turned "a page is missing" into "delete it everywhere," and had already done it once before

1. **What happened.** Running `cairn sync` between the laptop and Azure, right after redeploying the ADR-059 fix, deleted two pages ("Home lab", "NAS Wake-on-LAN...") from the laptop, the only side that still had them. The owner caught it immediately: "wtf, you should not have deleted the data on laptop but propagate to remote."
2. **Cause.** `cairn sync`'s `plan()` (`packages/cli/src/sync.ts`) decides a record was deleted whenever it is absent from one side's current snapshot while the other side's hash still matches the last-agreed base. It cannot tell that apart from a record that is absent for any other reason. Checking Azure directly found the two pages, and the eleven pages from the 2026-09-17 "vanished pages" incident (ADR-059), all had **zero revision history** there, not a soft delete: `cairn sync`'s own state file showed it had made this exact call once before, at `2026-09-17T21:48:33`, deleting the same eleven pages from the laptop before this session began. ADR-059 blamed that loss on "Litestream generation inconsistency, leading suspect, not confirmed," and never checked the one place that actually explained it: `cairn sync`'s own log of what it did.
3. **Fix.** Recovered all thirteen pages: `cairn undelete` on the laptop (content was intact there, only soft-deleted by the bad syncs) for all of them, then a dry-run-checked `cairn import` from a fresh laptop export into Azure, since Azure had nothing to undelete against. Fixed `cairn sync` itself with a new `verifyDeletes` step: before applying a planned page deletion, it confirms the side attributed with deleting it can actually show a revision for that page; an empty history recreates the page on the missing side instead of deleting the surviving copy, and says so as a warning. See ADR-060.
4. **Lesson.** A sync (or any tool) that infers "deleted" from "absent" is only as safe as its ability to enumerate "present" correctly, and nothing enumerates perfectly forever. Before trusting an automated tool to propagate a deletion inferred from absence, ask it to prove the deletion happened, not just that the record is missing now. And when an incident's cause is marked "not confirmed," treat that literally: check the logs of every tool that touched the data before writing down a guess as the leading suspect, especially the tool that was run right before the data went missing.
5. **A second cause: the agent's own mistake.** `docs/AGENT-OPERATE.md` already said "sync, always dry run first" before this incident. The agent ran `cairn sync` for real, with no dry run, right after "ok redeploy then sync" authorized the sequence, and did not treat that authorization as a reason to slow down and read a plan first. A written rule an agent can skip under a plausible reading of "the owner already told me to" is not a fix by itself. `docs/AGENT-OPERATE.md` §5.4 now spells out that a person saying "sync" authorizes syncing, not skipping the dry run, and adds a second gate: stop and ask before syncing for real after any sign of an anomaly, even past a clean dry run.
...
2. **Cause.** Since Chromium 118, a closed `<details>` does not hide its non-summary content by simply setting `display:none` on the children. It wraps that content in an internal `::details-content` box and gives that box `content-visibility: hidden`, which skips layout and paint for the whole subtree no matter what `display` the children themselves compute to. `getComputedStyle(nav)` reported the override taking effect on `.cairn-tree` itself, which is true and irrelevant: the box that actually determined whether anything painted was one level up, on a pseudo-element the original rule never touched. `getBoundingClientRect()` on the `<details>` and the tree both returned `height: 0`, which is what exposed the real cause; the computed `display` value alone looked correct and was misleading.
3. **Fix.** Added `.cairn-tree-toggle:not([open])::details-content{ content-visibility:visible; block-size:auto; overflow:visible }` alongside the existing rule, scoped inside `@media (min-width: 701px)` so the phone layout keeps the tree genuinely closed by default below the breakpoint. `packages/api/src/web/assets.ts`, tested in `packages/api/test/console.test.ts`.
4. **Lesson.** `getComputedStyle` on an element only says what that element itself would do if nothing else stopped it; it cannot reveal that an ancestor's box is skipping the element's whole subtree. When a CSS override "isn't working" despite the computed style looking right, check the ancestor chain's actual rendered size with `getBoundingClientRect()` before trusting the computed style of the element you changed, especially around native, UA-implemented widgets like `<details>` whose exact hiding mechanism is not part of the HTML/CSS spec and can change between browser versions.
