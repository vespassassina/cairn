...
### The Litestream replica stopped advancing, silently, for 59.5 hours, and every restart in that window reverted to the stale point

1. **What happened.** After ADR-060 shipped, redeployed, and a clean sync, the owner reported the same Home Assistant pages missing from Azure again: "i again have lost the online docs on home assistant collection." `cairn sync`'s state file showed it had not run since before the redeploy, ruling out the tool ADR-060 had just fixed.
2. **Cause.** Litestream writes compacted files at fixed intervals (level 1 every 30s, level 2 every 5 min) whenever the database is changing and replication is healthy. Listing the replica's Azure Blob container end to end found no level-1 or level-2 files between `2026-09-15T20:40:00Z` and `2026-09-18T08:18:36Z`, a 59.5-hour gap, despite the app being demonstrably active for most of it (the whole ADR-059 incident happened inside this window). Replication to the replica had stalled; the local database kept changing but none of it ever reached blob storage. Every container restart in that window restored from the stale `2026-09-15T20:40` point, discarding everything since, silently, as many times as the container restarted. This is one incident, not two: it explains the original 2026-09-17 loss (ADR-059) and this redeploy's repeat loss of the same pages the same way.
3. **Fix.** No code fix: this is an infrastructure question, not a bug in Cairn's own logic (ADR-060's sync fix and today's row extension stand on their own). Documented as ADR-061, with a recommendation, not yet acted on, to wire up a Log Analytics workspace for the Container App so a future stall leaves a trail past the current revision's console log.
4. **Lesson.** A successful restore only proves the restored file is internally consistent (ADR-051's integrity check); it says nothing about whether the replica it came from was still advancing. When data goes missing on a system that restores from a replica on every start, check whether the replica itself was still receiving writes during the loss window before looking anywhere else, the same way this entry's timestamp comparison did. And: a redeploy or restart soon after a manual data recovery is exactly the moment a stale replica would eat the recovery. Confirming fresh replica activity after a recovery, not just that the write succeeded, should be part of calling a recovery done.
