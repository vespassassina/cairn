...
#
# Not a source build of an unreleased fix (ADR-063 supersedes ADR-062 on this
# point): the source build at PR #1514's commit resolved issue #1515's stall,
# but its own snapshot writer produced a corrupt L9 snapshot ("nonsequential
# page numbers in snapshot transaction") on its very first Azure restart,
# which made Cairn's own integrity fallback roll back further than the real
# stall ever had, losing 13 pages outright. An unreleased branch traded a
# silent-stall bug for a silent-corruption bug that is strictly worse. Back
# to the checksummed release until a tagged Litestream version carries a
# reviewed fix for #1515.
